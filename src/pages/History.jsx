...
const History = () => {
  ...
  const totalTaxInView = scans.reduce(
    (sum, scan) => sum + (scan.totalGstAmount || 0),
    0,
  );
  const totalSpentInView = scans.reduce(
    (sum, scan) => sum + (scan.pricePaid || 0),
    0,
  );

  if (loading)
    return (
      <div className="p-10 text-center font-bold text-indigo-600 animate-bounce">
        Fetching History...
      </div>
    );
    ...
    <div className="p-6 max-w-2xl mx-auto min-h-screen bg-white">
      <header className="mb-6">
        ...
        <p className="text-gray-500 text-sm font-medium">
          Detailed breakdown of your tax contributions.
        </p>
      </header>
      ...
      <div className="bg-gray-900 text-white p-6 rounded-3xl mb-10 shadow-xl flex justify-between items-center">
        ...
        <div className="text-right border-l border-gray-700 pl-6">
          ...
          <p className="text-xl font-bold">
            ₹{totalSpentInView.toLocaleString()}
          </p>
        </div>
      </div>
      ...
      <div className="space-y-6">
        {scans.length > 0 ? (
          scans.map((scan) => (
            <div
              key={scan._id}
              className="flex gap-4 items-start border-b pb-6 border-gray-100 last:border-0"
            >
